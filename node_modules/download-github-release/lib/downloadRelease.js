...
function downloadRelease(user, repo, outputdir) {
  var filterRelease = arguments.length > 3 && arguments[3] !== undefined ?
    arguments[3] : pass;
  var filterAsset = arguments.length > 4 && arguments[4] !== undefined ?
    arguments[4] : pass;
  var leaveZipped = arguments.length > 5 && arguments[5] !== undefined ?
    arguments[5] : false;
  var p = arguments.length > 6 && arguments[6] !== undefined ? arguments[6] :
    false;
  ...
  return (0, _getReleases2.default)(user, repo).then(function(releases) {
    ...
    if (!release) {
      throw new Error('could not find a release for ' + user + '/' + repo +
        ' (' + _os2.default.platform() + ' ' + _os2.default.arch() + ')');
    }
    ...
    var promises = release.assets.map(function(asset) {
      var bar = bars.newBar((0, _rpad2.default)(asset.name, 24) +
        ' :bar :etas', {
          complete: '▇',
          incomplete: '-',
      ...
      return (0, _download2.default)(asset.browser_download_url, dest,
        p).then(function() {
        if (!leaveZipped && /\.zip$/.exec(destf)) {
          return (0, _extract2.default)(destf, outputdir).then(
            function() {
